  ...
  return (
    <form className="form">
      ...
      <div className="form__item">
        ...
        <div
          className={`form__formats ${
            activeFormat.length > 0 ? "form__formats--choice" : ""
          }`}
        >
          {Object.keys(formats).map((key) => (
            <button
              className={`format format--${key} ${
                key == activeFormat ? "format--active" : ""
              }`}
              key={key}
              type="button"
              onClick={() => {
      ...
      <div className="form__item">
        ...
        <p className="form__bottom-text">
          Учитывает URL, название, формат и размер
        </p>
      </div>
      <div className="form__item">
