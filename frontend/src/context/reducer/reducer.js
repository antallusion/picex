    ...
    case SORT: {
      ...
      if (format) {
        ...
        images = images.filter(
          (item) => item.size.format == state.activeFormat
        );
      }
...
function setImages(images) {
  ...
  for (let i = 0; i < arrCount; i++) {
    const array = [];
    if (i == arrCount - 1) {
      array.push(...images.slice(index));
    } else array.push(...images.slice(index, index + 48));
    index += 48;
    result.push(array);
  }
...
function sortImages(images, value) {
  ...
  } else if (value == "По высоте") {
    images.sort((a, b) => b.size.height - a.size.height);
  } else if (value == "По длине") {
    images.sort((a, b) => b.size.width - a.size.width);
  } else {
    images.sort((a, b) => b.size.size - a.size.size);
  }
