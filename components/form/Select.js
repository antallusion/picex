...
function List({ selected, onClose, opened, button, setSelected }) {
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (ref.current && !ref.current.contains(e.target) && e.target !== button) {
        onClose();
      }
    };
    ...
    <div className="form__select-list" ref={ref}>
      {items.map((item) => (
        <button
          ...
          className={`form__list-button ${item === selected ? 'form__list-button--active' : ''}`}
        >
          <span>{item}</span>
          {item === selected && <Sprite icon="select-check" width="14" height="14" />}
        </button>
      ))}
    </div>
  );
