...
export default function Loading() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (!message) {
        setCount((count) => {
          if (count + 1.8 > 15) setMessage(true);
          return count + 1.8;
        });
      }
      ...
      if (width + randNum >= 50 && index < 1) setIndex(index + 1);
      else if (width + randNum >= 90 && index < 2) setIndex(index + 1);
    ...
    <>
      <div className="search__loading">
        ...
        <div className="search__line">
          <div className="search__line-item" style={{ width: `${width}%` }} />
        </div>
      </div>
      {message && (
